...
class VentaGuayaquil extends Model
{
    protected string $connectionName = 'bdd_guayaquil';
    ...
    {
        $pdo = $this->getConnection($this->connectionName);
        $stmt = $pdo->prepare("SELECT * FROM {$this->table} WHERE id_ventas_guayaquil = :id");
        $stmt->execute(['id' => $id]);
    ...
    {
        $pdo = $this->getConnection($this->connectionName);
        $stmt = $pdo->prepare("DELETE FROM {$this->table} WHERE id_ventas_guayaquil = :id");
